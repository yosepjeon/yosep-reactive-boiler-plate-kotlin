...
@EnableR2dbcRepositories(
    basePackages = ["com.yosep.server.infrastructure.db.common.read.repository"],
    entityOperationsRef = "slaveEntityOperations",
)
class SlaveDBConfig(
    @param:Value("\${spring.r2dbc.slave.url}") private val url: String,
    @param:Value("\${spring.r2dbc.slave.username}") private val username: String,
    @param:Value("\${spring.r2dbc.slave.password}") private val password: String,
) {
    @Bean
    @Qualifier("slave")
    ...
    @Bean
    fun slaveEntityOperations(
        @Qualifier("slave") connectionFactory: ConnectionFactory,
    ...
    @Bean
    fun slaveTransactionalOperator(
        @Qualifier("slave") connectionFactory: ConnectionFactory,
